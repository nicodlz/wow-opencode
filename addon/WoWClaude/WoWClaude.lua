-- WoW OpenCode. Legacy WoWClaude globals keep the upstream transport compatible.
--
...
local ROLE_STYLE = {
	...
	claude = { label = "OpenCode", color = { 1.00, 0.82, 0.25 }, bg = { 0.85, 0.70, 0.30, 0.10 } },
...
local function InitDB()
	...
	local s = db.settings
	if s.autoRefresh == nil then s.autoRefresh = false end
...
function WoWClaude.BridgeState()
	if run.backend and not run.backend.healthy then
		return "down", 0.9, 0.25, 0.25, "OpenCode: " .. Display(run.backend.message)
	end
...
function WoWClaude.UpdateConnect()
	...
	local c = ActiveChat()
	ui.send:SetText(c and c.request and c.request.kind == "question" and "Answer" or c and c.pendingId and "Save draft" or "Send")
...
local function ApplyReplies(replies)
	...
	for _, r in ipairs(replies or {}) do
		...
		if c and c.pendingId and r.id == c.pendingId and (not r.token or r.token == "" or r.token == db.session) then
			matched = true
			MarkAcked(r.id)
			...
			elseif r.status == "working" then
				c.progress = r.text
				c.request = r.request
				if r.request and c.notifiedRequest ~= r.request.id then
					c.notifiedRequest = r.request.id
					c.unread = (c.unread or 0) + 1
					WoWClaude.Notify(c, "OpenCode needs your " .. (r.request.kind == "permission" and "permission" or "answer") .. ". Open the session to continue.")
				end
			end
		end
	end
...
-- Controls have their own IDs and acknowledgements, so a permission response or
-- folder browser never overwrites an in-flight prompt.
function WoWClaude.Control(op, text, chat)
	local c = chat or ActiveChat()
	if not c then return end
	for id, pending in pairs(db.controls) do
		if pending.chat == c.id and pending.op == op and (pending.text == (text or "") or op == "open" or op == "attach") then return id end
	end
	db.lastSeq = db.lastSeq + 1
	local id = db.lastSeq
	local requestID = (op == "permission" or op == "question") and c.request and c.request.id or nil
	local flags = "op=" .. op .. (requestID and (";req=" .. requestID) or "")
	local rec = { chat = c.id, cwd = c.cwd, flags = flags, name = c.name, text = text or "", sentAt = GetTime() }
	db.controls[id] = { chat = c.id, op = op, text = text or "", cwd = c.cwd, requestID = requestID }
	run.outbound[id] = rec
	run.controlPollAt = GetTime() + 2
	if db.settings.mode == "reload" then
		db.outbox = { id = id, session = db.session, chat = c.id, cwd = ToHex(c.cwd), text = ToHex(text or ""), op = op, requestID = requestID }
		SafeReload()
	else RefreshStrip() end
	return id
end

local function ApplyControlData(data)
	if type(data) ~= "table" then return end
	if data.backend then run.backend = data.backend end
	for _, r in ipairs(data.controls or {}) do
		local pending = db.controls[r.id]
		if r.token == db.session and pending then
			db.controls[r.id] = nil
			run.outbound[r.id] = nil
			local c = FindChat(r.chat)
			if c then
				if r.error then
					AddHistory(c, "system", r.error)
...
function WoWClaude.CurrentFolder() return ChatFolder(ActiveChat()) end

function WoWClaude.OpenWorkspace(folder, session)
	local c = ActiveChat()
	if c and (c.pendingId or #c.history > 0 or c.sessionID) then
		c = AddChat(nil, folder)
		if not c then print("WoW OpenCode: close a chat first (16 maximum).") return end
		WoWClaude.SwitchChat(c.id)
	end
	if not c then return end
	c.cwd = folder
	WoWClaude.Control(session and "attach" or "open", session or folder, c)
	WoWClaude.Toggle(true)
end

function WoWClaude.Respond(reply)
	local c = ActiveChat()
	if c and c.request then WoWClaude.Control(c.request.kind, reply, c) end
end

function WoWClaude.Stop()
	local c = ActiveChat()
	if c and c.pendingId then
		WoWClaude.Control("abort", "", c)
		c.progress = "Stopping OpenCode…"
		WoWClaude.Render()
	end
end
...
local function TryLoadSlot(why)
	...
	run.lastStreamPoll = GetTime()
	run.slotsUsed = (run.slotsUsed or 0) + 1
...
local function Tick()
	...
	if next(db.controls) and now >= (run.controlPollAt or 0) then
		run.controlPollAt = now + 3
		TryLoadSlot("control")
		return
	end
	local changed = false
	...
	local interval = (ui.frame and ui.frame:IsShown()) and 3 or 10
	if (run.slotsUsed or 0) >= SLOT_COUNT - 20 then interval = 15 end
	if now - (run.lastStreamPoll or now) >= interval then
		TryLoadSlot("stream")
		return
	end
	local moved = false
...
function WoWClaude.Send(text, allow)
	...
	if not c then return end
	text = Trim(text or "")
	if c.request and c.request.kind == "question" and text ~= "" then
		WoWClaude.Respond(text)
		return
	end
	if c.pendingId then
	...
	for _, pending in pairs(db.controls) do
		if pending.chat == c.id and (pending.op == "open" or pending.op == "attach") then
...
function WoWClaude.SayHello()
	...
	for id, pending in pairs(db.controls) do
		run.outbound[id] = { chat = pending.chat, cwd = pending.cwd, flags = "op=" .. pending.op .. (pending.requestID and (";req=" .. pending.requestID) or ""), text = pending.text, name = "", sentAt = now }
	end
...
function WoWClaude.SetFolder(rest, c)
	...
	if not c then return end
	if c.pendingId then
		AddHistory(c, "system", "Stop this session before changing its folder.")
		WoWClaude.Render()
		return
	end
	rest = Trim(rest or "")
...
function WoWClaude.UpdateStatus()
	...
	if c and c.pendingId then
		...
		if mode == "pixel" then
			...
			elseif run.slotsExhausted then
				s = "Live slots used up — click Reload when convenient to continue receiving updates"
			...
			elseif c.progress or (run.act and run.act[c.id] and run.act[c.id].count > 0) then
				s = "OpenCode is working on #" .. id .. " - " .. ActivityLine(c)
	...
	if ui.title then
		local t = c and Display(c.name) or "OpenCode"
...
local function GetBubble(i)
	...
	b.allow:SetScript("OnClick", function(self)
		if self.request then WoWClaude.Respond("once") else WoWClaude.Allow(self.chatId, self.rules) end
	end)
	b.allow:Hide()
	b.reject = CreateFrame("Button", nil, b, "UIPanelButtonTemplate")
	b.reject:SetSize(80, 22)
	b.reject:SetPoint("LEFT", b.allow, "RIGHT", 6, 0)
	b.reject:SetText("Reject")
	b.reject:SetScript("OnClick", function() local c = ActiveChat(); if c and c.request then WoWClaude.Respond(c.request.kind == "question" and "/reject" or "reject") end end)
	b.reject:Hide()
...
function WoWClaude.Render()
	...
	if ui.content and c then
		local follow = (ui.scroll:GetVerticalScroll() or 0) >= ui.scroll:GetVerticalScrollRange() - 32
		local width = ui.scroll:GetWidth()
		if not width or width < 80 then width = 400 end
		ui.content:SetWidth(width)
		local y, n = 0, 0
		local function Place(role, text, when, dim, denied, request)
			n = n + 1
			local b = GetBubble(n)
			...
			if request then
				b.allow:SetText(request.kind == "permission" and "Allow once" or "Answer below")
				b.allow:SetWidth(120)
				b.allow.request = request.kind == "permission"
				b.allow:SetShown(request.kind == "permission")
				b.reject:Show()
				extra = 28
			end
		...
		if c.pendingId then
			...
			if request then
				if request.kind == "permission" then
					p = "Permission: " .. tostring(request.permission) .. "\n" .. table.concat(request.patterns or {}, "\n")
				else
					local lines = { "OpenCode needs your answer. One line per question; numbers or text. Use commas for multiple choices." }
					for i, q in ipairs(request.questions or {}) do
						table.insert(lines, "\n" .. i .. ". " .. q.question)
						for n, option in ipairs(q.options or {}) do table.insert(lines, "  " .. n .. ") " .. option.label .. " — " .. (option.description or "")) end
					end
			...
			Place("claude", (p and p ~= "") and (head .. "\n\n" .. p) or head, "", not request, nil, request)
...
local function EchoToChat(chat, text)
	...
	local prefix = "|cff7ec8ff[OpenCode · " .. Display(chat.name) .. "]|r "
...
function WoWClaude.Notify(chat, text)
	...
	if UIErrorsFrame then
		UIErrorsFrame:AddMessage((chat.request and "OpenCode needs you in " or "OpenCode replied in ") .. Display(chat.name), 0.5, 0.8, 1, 1)
	end
end
...
local function PaintClaudeHeader(eb, chat)
	...
	header:SetWidth(0)
	header:SetText("To OpenCode [" .. Display(chat.name) .. "]: ")
...
local function BuildUI()
	...
	mini:SetScript("OnEnter", function(self)
		...
		GameTooltip:AddLine("OpenCode keeps working; the bar shows replies and requests for your attention.", 0.8, 0.8, 0.8, true)
		GameTooltip:Show()
	end)
	...
	local newBtn = MakeButton(panel, "+ New session", PANEL_W - 16, function() WoWClaude.OpenWorkspace(WoWClaude.CurrentFolder()) end)
	newBtn:SetPoint("TOP", panel, "TOP", 0, -8)
	local folders = MakeButton(f, "Folders", 80, function() WoWClaude.Browse("folders") end)
	folders:SetPoint("TOPRIGHT", f, "TOPRIGHT", -128, -8)
	title:SetPoint("RIGHT", folders, "LEFT", -10, 0)
	title:SetWordWrap(false)
	local sessions = MakeButton(f, "Sessions", 86, function() WoWClaude.Browse("sessions") end)
	sessions:SetPoint("LEFT", folders, "RIGHT", 4, 0)
	...
	input:SetScript("OnEnterPressed", function(self)
		if IsShiftKeyDown and IsShiftKeyDown() then self:Insert("\n") else WoWClaude.SendFromInput() end
	end)
	input:SetScript("OnTextChanged", function(self, userInput)
		if userInput then local c = ActiveChat(); if c then c.draft = self:GetText() end end
	end)
	...
	resend:Hide()
	ui.resend = resend
	local stop = MakeButton(f, "Stop", 65, WoWClaude.Stop)
...
SLASH_WOWCLAUDE1 = "/wow-opencode"
SLASH_WOWCLAUDE2 = "/oc"
SLASH_WOWCLAUDE3 = "/wow-claude"
SlashCmdList["WOWCLAUDE"] = function(msg)
	...
	elseif cmd == "reset" then
		c.resetNext = true
		AddHistory(c, "system", "Next message starts a fresh OpenCode session in " .. c.cwd)
		WoWClaude.Render()
		WoWClaude.Toggle(true)
	elseif cmd == "context" or cmd == "ctx" then
		...
		AddHistory(c, "system", (s.context
			and "Game context is ON: OpenCode receives your character and location. /oc context off to stop.\n\n"
			or "Game context is OFF. /oc context on to send this:\n\n") .. ctx
			.. "\n\nTip: shift-click an item, spell or quest to include its tooltip.")
		WoWClaude.Render()
		WoWClaude.Toggle(true)
	...
	elseif cmd == "help" then
		AddHistory(c, "system", "/oc folders — browse and open a project\n/oc sessions — resume an OpenCode session\n/oc cancel — stop the running task\n" .. HELP:gsub("wow%-claude", "wow-opencode"):gsub("Claude", "OpenCode"))
		WoWClaude.Render()
		WoWClaude.Toggle(true)
	else
		WoWClaude.Send(msg)
